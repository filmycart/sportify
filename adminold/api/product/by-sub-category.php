...
if(Helper::is_post()){
    ...
    if($api_token){
        ...
        if(!empty($setting)){
            ...
            $sub_category_id = Helper::post_val("sub_category_id");
            ...
            if($sub_category_id){
                ...
                if($page) {
                    ...
                    $products = $products->where(["sub_category_id"=>$sub_category_id])->andWhere(["status"=>1])
                        ->orderBy("id")->desc()->limit($start, API_PAGINATION)->all();

                }else $products = $products->where(["sub_category_id"=>$sub_category_id])
                    ->andWhere(["status"=>1])->orderBy("id")->desc()->all();
