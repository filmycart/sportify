            ...
            if($user_id && $product_id && $inventory_id){
				...
				if(!empty($product_from_db)){
					...
					if(empty($cart_from_db)){
						...
						if($cart->id > 0) {
						    $cart->create_property("adding_status", 1);
						    $response->create(200, "Success.", $cart);
						    
						} else $response->create(201, "Something went wrong. Please try again.", null);
					
					}else {
					    $cart_from_db->create_property("adding_status", 2);
					    $response->create(200, "Product already added to the cart.", $cart_from_db);
					}
