    ...
    if($api_token){
        ...
        if(!empty($setting)){
            ...
            if($category_id){
                ...
                $sub_categories = $sub_categories->where(["category_id" => $category_id])->all();
                ...
                if(!empty($sub_categories)){
                    foreach ($sub_categories as $sub_cat) {
                        ...
                        if($page) {
                            ...
                            $products = $products->where(["sub_category_id"=>$sub_cat->id])->andWhere(["status"=>1])
                                ->orderBy("id")->desc()->limit($start, API_PAGINATION)->all();

                        }else $products = $products->where(["sub_category_id"=>$sub_cat->id])->andWhere(["status"=>1])->orderBy("id")->desc()->all();
