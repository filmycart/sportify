...
if(!empty($admin)){
    ...
    if(Helper::is_get() && isset($_GET["id"])){
        ...
        //if($event->prev_price < 0) $event->prev_price = null;

       /* $inventories = new Inventory();
        $inventories = $inventories->where(["event" => $event->id])->all();
    ...
    <div class="main-content">
        <div class="item-wrapper one">
            ...
            <div class="item">
                ...
                <form data-validation="true" action="../private/controllers/event.php" method="post" enctype="multipart/form-data">
                    <div class="item-inner">

                        <div class="item-header">
                            <h5 class="dplay-inl-b">Events</h5>

                            <h5 class="float-r oflow-hidden">
                                <label class="status switch">
                                    <input type="checkbox" name="status"
                                        <?php if($event->status == 1) echo "checked"; ?>/>
                        ...
                        <div class="item-content">
                            <input type="text" name="id" value="<?php echo $event->id; ?>">
                            <input type="text" name="admin_id" value="<?php echo $event->admin_id; ?>">
                            <input type="text" name="prev_image" value="<?php echo ($event->image_name)?$event->image_name:''; ?>"/>
                            ...
                            <div class="image-upload">
                                ...
                                <img src="<?php if(!empty($product->image_name))
                                    echo UPLOADED_FOLDER . DIRECTORY_SEPARATOR . ($event->image_name)?$event->image_name:''; ?>" alt="" class="uploaded-image"/>
                                ...
                                <input type="hidden" name="image_resolution" value="<?php echo ($event->image_resolution)?$event->image_resolution:''; ?>"/>
                            </div>

                            <label>Title</label>
                            <input type="text" data-required="true" placeholder="Site Title" name="title" value="<?php echo isset($product->title)?$product->title:''; ?>"/>
                            ...
                            <div class="oflow-hidden w-100">
                                <div class="input-6 pr-7-5">
                                    ...
                                    <div class="price-input">
                                       <!--  <span><?php echo  $panel_setting->currency_font; ?></span> -->
                                        <input type="text" data-required="numeric" placeholder="Start Date" name="start_date" value="<?php echo isset($product->start_date)?$product->start_date:''; ?>"/>
                                    </div>
                                </div>
                                ...
                                <div class="input-6 pl-7-5">
                                    ...
                                    <div class="price-input">
                                        <!-- <span><?php echo  $panel_setting->currency_font; ?></span> -->
                                        <input type="text" placeholder="End Date" name="end_date" value="<?php echo isset($product->end_date)?$product->end_date:''; ?>"/>
                                    </div>
                                </div>
                            </div><!--oflow-hidden-->
                            <?php if($all_sub_categories > 0){ ?>
                                <select name="sub_category_id" data-required="dropdown">
                                    <option selected="true" disabled="disabled">Please select a category</option>
                                    <?php foreach($all_sub_categories as $item){ ?>
                                        <?php if($product->sub_category_id == $item->id) $selected_cat = "selected";
                                            else $selected_cat = ""; ?>
...
<!--                             <h5 class="mt-10 mb-30 oflow-hidden">
                            ...
                            <textarea data-required="true" class="desc" type="text" name="description"
                                          placeholder="Description"><?php echo isset($product->description)?$product->description:''; ?></textarea>
                                    ...
                                    <div class="combined-attr-inner">
                                        ...
                                        <?php
                                            ...
                                            foreach ($attr_arr as $attr_item){
											    ...
											    $attr_item_id = "";													
												if((isset($attr_item_id_arr[1])) && (!empty($attr_item_id_arr[1]))) {	
