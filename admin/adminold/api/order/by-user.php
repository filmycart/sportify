            ...
            if($user_id){
                ...
                if (count($orders) > 0) {
                    foreach ($orders as $item){
                        ...
                        $tax_final = (string) round(($order->amount - $total_price), 2);
                        
                        
                        
                        $order->create_property("tax", $tax_final);
                        $order->create_property("ordered_products", $ordered_products_arr);
                        
                        $order_response =  $order->to_valid_array();
                        
                        if($tax_final < 1) $order_response["tax"] = 0;
                        
                        array_push($order_arr, $order_response);
                    }
