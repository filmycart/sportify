    ...
    if (Helper::is_post()) {
        ...
        if(empty($_POST['id'])){
            ...
            if(!$message->is_empty()){
                ...
                Helper::redirect_to("../../".ADMIN_FOLER_NAME."/categories.php");
            }else if(!$errors->is_empty()){
                Session::set_session($errors);
                Helper::redirect_to("../../".ADMIN_FOLER_NAME."/category-form.php");
            }
            ...
            if(!$message->is_empty()){
                ...
                Session::set_session($errors);
                Helper::redirect_to("../../".ADMIN_FOLER_NAME."/category-form.php?id=" . $category->id);
            }
        }
            ...
            if($admin->id == $category->admin_id){
                ...
                if(count($category_from_db) > 0){
                    ...
                    if($category->where(["id" => $category->id])->andWhere(["admin_id" => $category->admin_id])->delete()){
                        ...
                        $sub_category = $sub_category->where(["category_id" => $category->id])->all();


                        foreach ($sub_category as $sub_cat_item){

                            Upload::delete($sub_cat_item->image_name);

                            $products_of_cat = new Product();
                            $products_of_cat = $products_of_cat->where(["sub_category_id"=>$sub_cat_item->id])->all();
                            ...
                            foreach ($products_of_cat as $p_item){
                                ...
                                $products_of_sub_cat = new Product();
                                $products_of_sub_cat->where(["id"=>$p_item->id])->delete();

                            }

                            $sub_category_delete = new Sub_Category();
                            $sub_category_delete->where(["id" => $sub_cat_item->id])->delete();
